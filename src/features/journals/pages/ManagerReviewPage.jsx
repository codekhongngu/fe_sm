                          ...
                          {(selected?.form4Rows || []).map((row, idx) => (
                            <div key={idx} style={{ border: '1px solid #e5eaef', padding: 12, borderRadius: 8 }}>
                              ...
                              <div className="filters" style={{ marginBottom: 8 }}>
                                ...
                                <div><div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>Khó khăn/Lý do từ chối</div><textarea className="field" rows={2} value={row.customerIssue || ''} onChange={(e) => { const newRows = [...selected.form4Rows]; newRows[idx].customerIssue = e.target.value; setSelected(prev => ({ ...prev, form4Rows: newRows })); }} /></div>
                                <div><div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>Nguyên nhân sâu xa</div><textarea className="field" rows={2} value={row.consequence || ''} onChange={(e) => { const newRows = [...selected.form4Rows]; newRows[idx].consequence = e.target.value; setSelected(prev => ({ ...prev, form4Rows: newRows })); }} /></div>
                              </div>
                              <div className="filters" style={{ marginBottom: 8 }}>
                                <div><div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>Giải pháp</div><textarea className="field" rows={2} value={row.solutionOffered || ''} onChange={(e) => { const newRows = [...selected.form4Rows]; newRows[idx].solutionOffered = e.target.value; setSelected(prev => ({ ...prev, form4Rows: newRows })); }} /></div>
                                <div><div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>Đề xuất hỗ trợ</div><textarea className="field" rows={2} value={row.valueBasedPricing || ''} onChange={(e) => { const newRows = [...selected.form4Rows]; newRows[idx].valueBasedPricing = e.target.value; setSelected(prev => ({ ...prev, form4Rows: newRows })); }} /></div>
                              </div>
                              <div className="filters">
                                <div><div style={{ fontSize: 13, color: '#64748b', marginBottom: 4 }}>Kết quả</div><textarea className="field" rows={2} value={row.result || ''} onChange={(e) => { const newRows = [...selected.form4Rows]; newRows[idx].result = e.target.value; setSelected(prev => ({ ...prev, form4Rows: newRows })); }} /></div>
                              </div>
                            </div>
                          ))}
                        </div>
